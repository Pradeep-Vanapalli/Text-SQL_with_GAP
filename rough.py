...
def process_selected_value(selected_value: str):
    ...
    quest = ["What is the monthly churn rate", "What is Weekly churn rate", "What is the churn rate of current week" , "What is the churn rate of current month", "What is the churn rate of current year"]
    sql = ["SELECT o.month_name, ((c.total_customers - COUNT(DISTINCT o.customer_id)) / c.total_customers) * 100 AS monthly_churn_rate_percentage FROM (SELECT DATE_FORMAT(created_at, '%Y-%m') AS month_name, customer_id, id FROM orders WHERE DATE_FORMAT(created_at, '%Y') = DATE_FORMAT(NOW(), '%Y')) o CROSS JOIN (SELECT COUNT(DISTINCT id) AS total_customers FROM customers) c GROUP BY o.month_name, c.total_customers ORDER BY o.month_name",
    ...
    #selected_value = "What is the monthly churn rate"  # Example selected value, replace it with the actual user input
